...
'''
This is the template to use when adding a new dependency.

<DependencyURL> = "<URL>"

def Install<DependencyName>_Helper(context, force, buildArgs):
    # Provide backup sources for downloading <DependencyName> to avoid issues when
    # one mirror goes down.
    major, minor, patch = <DependencyName>_VERSION
    version = f"{major}.{minor}.{patch}"
    filename = f"<DependencyName>_{major}_{minor}_{patch}.zip"
    urls = [
        # The sourceforge mirror is typically faster
        # so we use that first.
        f"https://sourceforge.net/projects/boost/files/boost/{version}/{filename}/download",

    ]
    
def Install<DependencyName>(context, force, buildArgs):
    with CurrentWorkingDirectory(DownloadURL(<DependencyURL>, context, force)):
        RunCMake(context, force, buildArgs)
<DependencyName> = Dependency("<dependencyName>", Install<DependencyName>, "<file/to/check>")
AllDependencies.append(<DependencyName>)
AllDependenciesByName["<dependencyname>"] = <DependencyName>
